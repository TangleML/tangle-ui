...
export function SelectedNodesList({ nodes, spec }: SelectedNodesListProps) {
  ...
  return (
    <BlockStack gap="2">
      <Label className="text-muted-foreground">Selected Nodes</Label>
      <BlockStack gap="1" className="max-h-48 overflow-y-auto">
        {nodes.map((node) => (
          <InlineStack
            ...
            gap="2"
            blockAlign="center"
            className="text-xs py-1.5 px-2 bg-muted rounded border border-border"
          >
            <Icon
            ...
            />
            <Text size="xs" className="text-foreground truncate flex-1">
