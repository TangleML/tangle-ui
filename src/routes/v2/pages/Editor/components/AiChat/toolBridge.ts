 ...
 *
 * Each method mutates the LIVE MobX `ComponentSpec` from the editor session
 * inside an undo group, so the agent's edits are user-visible immediately
 * and undoable as a single step. The worker proxies these methods via
 * Comlink — no command serialization or tempId remapping is needed.
 */
import type {
  ConnectArgs,
  ToolBridgeApi,
 ...
 * exposed to the worker via `Comlink.expose()`. Methods throw when the
...
export function createToolBridge(deps: BridgeDeps): ToolBridgeApi {
  ...
  return {
    ...
    async addTask({ name, componentRef }) {
      const spec = requireSpec();
      const hydrated =
        (await hydrateComponentReference(componentRef as ComponentReference)) ??
