...
type ComponentsSearch = { component?: string };
...
const CollapsibleSection = ({
  ...
  return (
    <>
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center gap-1.5 px-3 py-1.5 bg-muted/40 border-b border-border/50 sticky top-0 z-10 hover:bg-muted/60 cursor-pointer"
      >
        <ChevronRight
...
const ComponentRow = ({
  ...
  <button
    ...
    className={`w-full text-left pr-3 py-1.5 border-b border-border/50 hover:bg-muted/50 flex flex-col gap-0 cursor-pointer ${
      selectedDigest && selectedDigest === component.digest ? "bg-muted" : ""
    }`}
  >
  ...
  return (
    <>
      <button
        onClick={() => setOpen((o) => !o)}
        style={{ paddingLeft: `${depth * 12 + 12}px` }}
        className="w-full flex items-center gap-1.5 pr-3 py-0.5 border-b border-border/50 bg-muted/10 hover:bg-muted/30 cursor-pointer text-left"
      >
        <ChevronRight
          className={`h-3 w-3 text-muted-foreground transition-transform shrink-0 ${
            open ? "rotate-90" : ""
...
const IORow = ({
  ...
  <div className="grid grid-cols-[1fr_auto_auto] items-start gap-x-3 gap-y-0.5 px-3 py-2 border-b border-border/50 last:border-0">
    ...
    {required ? (
      <span className="shrink-0 mt-0.5 inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-rose-100 text-rose-700 leading-none">
        req
      </span>
    ) : (
      <span className="shrink-0 mt-0.5 inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-muted text-muted-foreground leading-none">
        opt
      </span>
    )}
  </div>
...
export function DashboardComponentsView() {
  ...
  const search = useSearch({ strict: false }) as ComponentsSearch;
  const selectedDigest = search.component;
  const handleSelect = (component: ComponentReference) => {
    navigate({
      to: pathname,
      search: { ...search, component: component.digest },
    });
  };
