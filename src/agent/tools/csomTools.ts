 ...
 * the Comlink-proxied `ToolBridgeApi`. The bridge runs on the main thread
 * and mutates the live MobX `ComponentSpec` inside an undo group, so the
 * agent's edits are immediately reflected in the editor and undoable as
