...
const DOCS_VECTOR_STORE_URL = "/agent-index/docs-vector-store.json";
...
export class DocsVectorStoreCache {
  ...
  async #fetchAndPersist(): Promise<PersistedVectorStore> {
    ...
    if (row && row.embeddingModel === expectedModel) {
      return row.payload;
    }
    ...
    const payload = (await response.json()) as PersistedVectorStore;
  ...
  try {
    ...
    if (store.vectors.length === 0) {
      const empty: SearchDocsResponse = {
        results: [],
        message:
          "Docs index is empty. Run `pnpm agent:index-docs` to populate it.",
      };
