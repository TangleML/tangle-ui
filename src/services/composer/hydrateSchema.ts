...
/**
 * Casts and validates a raw JSON import as a `ComposerSchema`.
 *
 ...
 * as `string` rather than `BlockType.TextBlock`). This function encapsulates
 * the `unknown` cast in one place so consumers never need `as unknown as`.
 *
 * Runtime safety is provided by `validateSchema()`, which checks structure,
 * block types, and placeholder consistency at module load time.
 *
 * Alternative: a wildcard `.d.ts` declaration (e.g.,
 * `declare module "*OverlaySchema.json"`) can type all matching JSON imports
 * automatically, but requires a naming convention.
 */
export function loadSchema(raw: unknown): ComposerSchema {
  const schema = raw as ComposerSchema;
...
function collectPlaceholders(value: unknown): Set<string> {
  ...
  function walk(v: unknown): void {
    if (typeof v === "string") {
      ...
      while ((match = PLACEHOLDER_RE.exec(v)) !== null) {
        tokens.add(match[1]);
      }
