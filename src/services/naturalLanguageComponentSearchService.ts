...
function buildRerankSystemPrompt(): string {
  return [
    ...
    "- Treat negative constraints (for example: no, not, without, cannot, must not, exclude) as hard requirements.",
    "- If the query says a title/name cannot contain a term, drop candidates whose name contains that term.",
    "- Do not reward a term that appears only because the user forbade it.",
