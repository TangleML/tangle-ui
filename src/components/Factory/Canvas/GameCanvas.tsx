...
const GameCanvas = ({ children, ...rest }: GameCanvasProps) => {
  ...
  const setupEdgesComplete = useRef(false);
  ...
  useEffect(() => {
    if (setupEdgesComplete.current || !reactFlowInstance || nodes.length === 0)
      return;

    const allNodesMeasured = nodes.every((node) => node.measured?.width);
    ...
    setupEdgesComplete.current = true;
