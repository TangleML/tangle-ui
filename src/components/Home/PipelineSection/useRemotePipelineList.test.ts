...
describe("useRemotePipelineList", () => {
  ...
  it("searches names outside loaded pages using the global summary catalog", async () => {
    const files = Array.from({ length: 25 }, (_, index) => file(index));
    files[24] = file(24, "Quarterly revenue");
    listPage.mockResolvedValue({
      files: files.slice(0, 10),
      nextPageToken: "page-two",
      totalCount: files.length,
    });
    listSummaries.mockResolvedValue(files);
    const { result } = renderList();
    await waitFor(() => expect(result.current.isPending).toBe(false));

    act(() => result.current.filterBarProps.setSearchQuery("REVENUE"));

    await waitFor(() => expect(result.current.rows).toHaveLength(1));
    expect(result.current.rows[0][0]).toBe(files[24].referenceId);
  ...
  it("sorts and paginates the full summary catalog rather than only the server page", async () => {
    const files = Array.from({ length: 12 }, (_, index) =>
      file(index, `Pipeline ${String(12 - index).padStart(2, "0")}`),
    );
    listPage.mockResolvedValue({
      files: files.slice(0, 10),
      nextPageToken: "page-two",
    ...
    expect(result.current.rows[0][1].name).toBe("Pipeline 01");
    act(() => result.current.pagination.goToNextPage());
    expect(result.current.rows.map(([, entry]) => entry.name)).toEqual([
      "Pipeline 11",
      "Pipeline 12",
    ]);
    expect(listPage).toHaveBeenCalledOnce();
  ...
  it("filters dates across the summary catalog without reading definitions", async () => {
    const old = file(0, "Older pipeline", new Date("2026-09-10T12:00:00Z"));
    const recent = file(1, "Recent pipeline");
    listPage.mockResolvedValue({
      files: [old],
      nextPageToken: "page-two",
      totalCount: 2,
    });
    listSummaries.mockResolvedValue([old, recent]);
    const { result } = renderList();
    await waitFor(() => expect(result.current.isPending).toBe(false));
    ...
    expect(listSummaries).toHaveBeenCalledOnce();
    expectNoDefinitions([old, recent]);
  });
