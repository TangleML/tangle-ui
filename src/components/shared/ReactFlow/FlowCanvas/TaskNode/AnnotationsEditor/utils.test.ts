...
describe("getCloudProviderConfig", () => {
  ...
  it("derives options from launcher keys with title as the display name", () => {
    const config = getCloudProviderConfig(
      makeSchema({
        "kubernetes_google_ml-offline-us-ce1-eo9": { title: "eo9" },
      }),
    );

    expect(config?.annotation).toBe(CLOUD_PROVIDER_ANNOTATION);
    expect(config?.options).toEqual([
      { value: "kubernetes_google_ml-offline-us-ce1-eo9", name: "eo9" },
    ]);
  });

  it("prefers x-label over title, and capitalizes the key when neither is set", () => {
    const config = getCloudProviderConfig(
      makeSchema({
        google: { "x-label": "Google Cloud", title: "ignored" },
        nebius: { title: "Nebius" },
        acme: {},
      }),
    );
  ...
  it("hides an option when its feature flag is disabled", () => {
    ...
    const config = getCloudProviderConfig(
      makeSchema({
        ...
        "kubernetes_google_ml-offline-us-ce1-eo9": {
          title: "eo9",
  ...
  it("qualifies the name by platform and captions the project from x- fields", () => {
    ...
    expect(config?.options?.[0]).toEqual({
      value: "kubernetes_google_ml-offline-us-ce1-eo9",
      name: "GKE: eo9",
      caption: "Project: shopify-ml-offline-prod",
    });
  });
...
describe("resolveLauncherKey", () => {
  const schema = makeSchema({
    "kubernetes_google_ml-offline-us-ce1-lt3": { "x-aliases": ["google"] },
    "kubernetes_google_ml-offline-us-ce1-eo9": {},
  });

  it("returns a current launcher key unchanged", () => {
    expect(
      resolveLauncherKey(schema, "kubernetes_google_ml-offline-us-ce1-lt3"),
    ).toBe("kubernetes_google_ml-offline-us-ce1-lt3");
  });

  it("maps a legacy key to its current launcher key", () => {
    expect(resolveLauncherKey(schema, "google")).toBe(
      "kubernetes_google_ml-offline-us-ce1-lt3",
    );
  });
...
describe("buildLauncherSchemaFromCapabilities", () => {
  ...
  it("copies the provider resource fields into properties verbatim and synthesizes the Nebius GPU enum", () => {
    ...
    expect(gkeEntry?.["x-provider"]).toBe("GKE");
  ...
  it("offers a section with no clusters map, with its own resource fields", () => {
    const built = buildLauncherSchemaFromCapabilities({
      acme: {
        label: "Acme",
        resource_fields: [GPU_FIELD],
    ...
    const entry = built.launcher_annotation_schemas?.["acme"];
    expect(entry?.title).toBe("Acme");
  ...
  it("leaves a section with no clusters unqualified by a provider", () => {
    const built = buildLauncherSchemaFromCapabilities({
      gke: gke("eo9"),
      acme: { label: "Acme" },
    } as LauncherConfig);

    const acme = built.launcher_annotation_schemas?.["acme"];
    expect(acme?.["x-provider"]).toBeUndefined();

    const options = getCloudProviderConfig(built)?.options ?? [];
    expect(options.map((option) => option.name)).toEqual(["GKE: eo9", "Acme"]);
  });

  it("shows a disabled section as unavailable rather than hiding it", () => {
    const built = buildLauncherSchemaFromCapabilities({
      acme: {
        label: "Acme",
        enabled: false,
    ...
    expect(entry?.["x-deprecated"]).toBe(true);
    expect(entry?.["x-deprecated-message"]).toBe(
      "Acme is not enabled for this deployment",
    );

    const option = getCloudProviderConfig(built)?.options?.find(
      (candidate) => candidate.value === "acme",
    );
...
describe("resolveClusterSelection", () => {
  ...
  it("never introduces an accelerator the task did not already have", () => {
    expect(resolveClusterSelection(schema, "h200")).toEqual({
      cloudProviderValue: "h200",
    });
  });

  // A section declaring its own accelerator enum must not pre-fill a GPU either: it
  // decides placement itself, so a selection is not a GPU request.
  it("does not pre-fill a GPU for a cluster that offers one from its own fields", () => {
    const withOwnEnum = buildLauncherSchemaFromCapabilities({
      acme: {
        label: "Acme",
        resource_fields: [
          { ...GPU_FIELD, enum: ["NVIDIA-H200", "NVIDIA-B300-SXM6-PC"] },
        ],
      },
    } as LauncherConfig);

    expect(resolveClusterSelection(withOwnEnum, "acme")).toEqual({
      cloudProviderValue: "acme",
    });
  });
  ...
  it("clears the accelerator when the target does not offer it", () => {
    ...
    ).toEqual({ cloudProviderValue: "b300", clearAccelerator: true });
  });

  it("clears the accelerator when the target has no GPU field at all", () => {
    expect(
      resolveClusterSelection(
        schema,
        "eo9",
    ...
    ).toEqual({ cloudProviderValue: "eo9", clearAccelerator: true });
  });

  it("follows the successor the cluster being left declares for the target", () => {
    const succeeding = buildLauncherSchemaFromCapabilities({
      nebius: nebius({
        h200: {
          accelerators: [
            {
  ...
  it("ignores a successor that names a cluster other than the target", () => {
    const succeeding = buildLauncherSchemaFromCapabilities({
      nebius: nebius({
        h200: {
          accelerators: [
...
describe("resolveSelectedClusterKey", () => {
  ...
  it("reads the cluster key from the annotation", () => {
    expect(
      resolveSelectedClusterKey(schema, {
        [CLOUD_PROVIDER_ANNOTATION]: "eo9",
      }),
    ).toBe("eo9");
  });
  ...
  it("passes an unknown value through unchanged", () => {
    expect(
      resolveSelectedClusterKey(schema, {
        [CLOUD_PROVIDER_ANNOTATION]: "azure",
      }),
    ).toBe("azure");
  });
});
...
describe("listedOptions", () => {
  const live = { value: "eo9", name: "GKE: eo9" };
  const retiring = {
    value: "lt3",
    name: "GKE: lt3",
    deprecated: true,
    deprecationMessage: "Retired. Use GKE: eo9",
  };

  it("leaves out a deprecated option when the task has none set", () => {
    expect(listedOptions([live, retiring], undefined)).toEqual([live]);
  });

  it("leaves out a deprecated option the task has not chosen", () => {
    expect(listedOptions([live, retiring], "eo9")).toEqual([live]);
  });
