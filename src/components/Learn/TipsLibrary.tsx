...
function TipCard({ tip }: { tip: Tip }) {
  return (
    <Card
      className="h-full py-4 gap-2 hover:border-primary/40 hover:shadow-md transition-all duration-200"
...
const grouped = TIP_CATEGORY_ORDER.filter((category) =>
  buckets.has(category),
).map((category) => ({ category, tips: buckets.get(category)! }));
