...
function isTourDifficulty(value: string): value is TourDifficulty {
  return TOUR_DIFFICULTY_ORDER.some((difficulty) => difficulty === value);
}

function parseTours(raw: typeof toursData): Tour[] {
  return raw.map((tour) => {
    if (!isTourDifficulty(tour.difficulty)) {
      throw new Error(
        `Invalid difficulty "${tour.difficulty}" for tour "${tour.id}". ` +
          `Expected one of: ${TOUR_DIFFICULTY_ORDER.join(", ")}.`,
      );
    }
    return { ...tour, difficulty: tour.difficulty };
  });
}

export const tours: Tour[] = parseTours(toursData);
